...
import data_informatica from "./db_juguetes.json";
import "./card.css";
import React, { useState } from "react";
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faCartShopping, faComment } from '@fortawesome/free-solid-svg-icons'

function CardProduc5() {
    const [, setSelectedProduct5] = useState(null);
            ...
            <Dropdown>
                ...
                <Dropdown.Menu>
                    {data_informatica.map((x, i) => (
                        <Nav.Link href={`#${x.afiliado}`} key={i} onClick={() => setSelectedProduct5(x.afiliado)} className="m-1">
                            {x.titulo}
                        </Nav.Link>
                    ))}
                </Dropdown.Menu>
